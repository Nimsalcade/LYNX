...
FARM_REFRESH_S     = 45.0      # How often to repost the full ladder
MAX_ORDER_AGE_S    = 60.0      # Cancel and repost if order older than this
...
# When a spike hits it sweeps the book and fills our high-price orders.
LADDER_STEPS = [
    0.10, 0.15, 0.20,           # Floor bids — cheap lottery (rare fills, huge ROI)
    0.25, 0.30, 0.35, 0.40,     # Low delta zone
    0.45, 0.50, 0.55,           # Core delta-neutral zone (fills most often)
    0.60, 0.65, 0.70,           # Upper mid zone
    0.75, 0.80, 0.85,           # Snipe layer — fills on spikes (no separate FOK needed)
]
LADDER_SHARES_PER_STEP = 5     # Small per rung — $0.50-$4.25 per order at each level
...
# --- Hard capital cap ---
# NEVER deploy more than $1,000 total across all 4 markets.
# Split evenly: $250 per window (both sides combined).
# Counts BOTH filled positions AND resting locked orders.
MAX_GLOBAL_CAPITAL_USD   = 1000.0   # Hard ceiling across all markets
...
class MakerLoop:
    ...
    async def _farm_orders(
        ...
        locked_resting = sum(
            o.price * o.shares for o in active_orders.values()
        )
